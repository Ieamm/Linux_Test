...
struct Sock
{
    ...
    static void Connect(int sock, std::string serverIp, uint16_t serverPort)
    {
        struct sockaddr_in s_addr;
        socklen_t len = sizeof(s_addr);
        memset(&s_addr, 0, len);
        if(connect(sock, (const struct sockaddr*)&s_addr, len) < 0)
        {
            logMessage(NOTICE, "Connect error sock[%d]\n",sock);
        }
    }
};